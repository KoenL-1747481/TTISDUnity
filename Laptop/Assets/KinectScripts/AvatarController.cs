...
{	
    ...
    {	
		...
		List<Quaternion> newBoneRotations = new List<Quaternion>();
		...
		for (var boneIndex = 0; boneIndex < bones.Length; boneIndex++)
		{
			...
			if(boneIndex2JointMap.ContainsKey(boneIndex))
			{
				...
				newBoneRotations.Add(getBoneRotation(boneIndex));
			}
    ...
    {
		return bones[boneIndex].rotation;
	}
	...
	protected void TransformBone(uint userId, KinectWrapper.NuiSkeletonPositionIndex joint, int boneIndex, bool flip)
    {
		...
		if(smoothFactor != 0f)
        	boneTransform.rotation = Quaternion.Slerp(boneTransform.rotation, newRotation, smoothFactor * Time.deltaTime); //@new bone rotation
		else
			boneTransform.rotation = newRotation;																		   //@new bone rotation
	}
	...
	protected void MoveAvatar(uint UserID)
	{
		...
		if (!offsetCalibrated)
		{
			...
			if(offsetRelativeToSensor)
			{
				...
				{
					transform.position = offsetPos; //set position @@@@@@@@@@@@@@@@@@
				}
			}
		}
